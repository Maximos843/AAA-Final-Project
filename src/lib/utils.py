...
def open_image(image_link: BufferedReader) -> np.array:
    image = np.asarray(Image.open(image_link))
    return image
...
def preprocess_image(image: np.array) -> torch.tensor:
    test_transform = A.Compose(
        [
            ...
            A.Normalize(mean=np.array(Variables.MEAN), std=np.array(Variables.STD)),
            ToTensorV2(),
        ]
    )
...
def convert_description_to_tokens(text: str, tokenizer, encode_mapping) -> np.array:
    morph = pymorphy3.MorphAnalyzer()
    text_tokens = tokenizer(text)
    text_tokens_normal_form = [morph.parse(token)[0].normal_form for token in text_tokens]
    text_tokens_enc = [encode_mapping.get(token, Variables.NOT_IN_VOCAB) for token in text_tokens_normal_form]
